...
- **Per-item result modal**. After every bulk mutation the toolbar fires a Sonner summary toast (`X of Y succeeded · Z failed`). The toast carries a "Details" action — the ONLY entry point to `BulkResultModal`. Auto-opening the modal on every run was tried and rejected as too disruptive (pulls focus before the user can read the toast). Clicking Details renders the per-item outcome list (success or `ApiError.code + message`); failed ids are copyable. Missing `summary` fields fall back to a computed count so the modal cannot crash on a malformed envelope.
