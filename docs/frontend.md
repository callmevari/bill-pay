...
- Where the backend exposes a bulk endpoint (`POST /bills/bulk/{approve,archive,edit}`, `POST /payments/bulk/{release,mark-as-paid,cancel}`), the toolbar calls it directly via `use-bulk-bill-mutations` / `use-bulk-payment-mutations`. Where it does NOT (`submit-for-approval`, `reject`, `schedule`, `retry`), the toolbar iterates via `Promise.allSettled` (`lib/bulk.ts → runClientBulk`) over the single-item hooks and shapes the result envelope identical to the native bulk responses so the per-item modal renders either source uniformly.
