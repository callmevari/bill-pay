...
- **Bill `paymentMethod` override**. `BillForm` gains a `Select` with "Use vendor default" (sends `null` on the wire to clear an existing override) plus the five `PaymentMethod` enum values. The form submits `null` to clear and `undefined` to leave the field alone; consumers of `useUpdateBillMutation` rely on that distinction. The bill detail header surfaces "Payment method (overrides vendor default): WIRE" only when `bill.paymentMethod && !bill.payment` — once the Payment row exists, the resolved method lives on the payment block (`payment.method`), so the pre-approve hint disappears to avoid two sources of truth on screen. The activity timeline reads `metadata.methodSource === 'bill'` on `payment.created` rows and appends a small "Bill override · METHOD" badge so the audit trail explains why a particular method won.
...
- The toolbar surface is tab-specific. Drafts → submit / edit / archive; For Approvals → approve / reject / archive; For Payment → schedule / release / mark paid / cancel / retry; History → hidden (the table never renders the selection column there); Overview → union of every action above (a mixed selection naturally hits eligibility=0 on some, so the per-status eligibility counter drives the disabled state). Buttons are double-gated: by `useCan(...)` matching `docs/api-contract.md → Permission matrix` (role-disallowed buttons are hidden entirely, not disabled), then by per-status eligibility against the current selection. If the active role has no applicable action on the current tab, `bills-page.tsx` stops forwarding `selectionState` to the table altogether and the checkbox column never renders — Viewer never sees it; Approver loses it on Drafts and For payment.
- Every action the toolbar exposes maps 1:1 to a native bulk endpoint on the backend: `POST /bills/bulk/{approve,archive,edit,submit-for-approval,reject}` and `POST /payments/bulk/{release,mark-as-paid,cancel,schedule,retry}`. The hook layer (`use-bulk-bill-mutations` / `use-bulk-payment-mutations`) is a thin `apiFetch` wrapper around each endpoint that returns the documented `{ results, summary }` envelope.
- **Per-item result modal**. After every bulk mutation the toolbar fires a Sonner summary toast (`X of Y succeeded · Z failed`). The toast carries a "Details" action — the ONLY entry point to `BulkResultModal`. Auto-opening the modal on every run was tried and rejected as too disruptive (pulls focus before the user can read the toast). Clicking Details renders the per-item outcome list (success or `ApiError.code + message`); failed ids are copyable. If the envelope arrives without a `summary` field, the modal simply omits the summary header rather than fabricating one — the per-item list still renders, so the user keeps the actionable information either way.
